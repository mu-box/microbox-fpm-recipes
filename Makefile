...
ubuntu:
	docker build -t mubox/fpm-cookery:ubuntu -f Dockerfiles/Dockerfile.ubuntu Dockerfiles

PHONY: ubuntu-18.04

ubuntu-18.04:
	docker build -t mubox/fpm-cookery:ubuntu-18.04 -f Dockerfiles/Dockerfile.ubuntu-18.04 Dockerfiles

PHONY: narc-docker

narc-docker: ubuntu
	docker build --build-arg NARC_VERSION=${NARC_VERSION} -t mubox/fpm-cookery:narc narc
...
pkg/deb/narc_${NARC_VERSION}_amd64.deb: pkg/deb narc-docker
	docker run --rm mubox/fpm-cookery:narc > pkg/deb/narc_${NARC_VERSION}_amd64.deb
...
libbframe-docker: ubuntu
	docker build --build-arg LIBBFRAME_VERSION=${LIBBFRAME_VERSION} -t mubox/fpm-cookery:libbframe libbframe
...
pkg/deb/libbframe_${LIBBFRAME_VERSION}_amd64.deb: pkg/deb libbframe-docker
	docker run --rm mubox/fpm-cookery:libbframe > pkg/deb/libbframe_${LIBBFRAME_VERSION}_amd64.deb
...
libmsgpack3-18.04-docker: ubuntu-18.04
	docker build --build-arg LIBMSGPACK3_VERSION=${LIBMSGPACK3_VERSION} -t mubox/fpm-cookery:libmsgpack3-18.04 -f libmsgpack3/Dockerfile-18.04 libmsgpack3
...
pkg/deb/libmsgpack3_${LIBMSGPACK3_VERSION}_18.04_amd64.deb: pkg/deb libmsgpack3-18.04-docker
	docker run --rm mubox/fpm-cookery:libmsgpack3-18.04 > pkg/deb/libmsgpack3_${LIBMSGPACK3_VERSION}_18.04_amd64.deb
...
libmsgxchng-docker: ubuntu
	docker build --build-arg LIBMSGXCHNG_VERSION=${LIBMSGXCHNG_VERSION} -t mubox/fpm-cookery:libmsgxchng libmsgxchng
...
pkg/deb/libmsgxchng_${LIBMSGXCHNG_VERSION}_amd64.deb: pkg/deb libmsgxchng-docker
	docker run --rm mubox/fpm-cookery:libmsgxchng > pkg/deb/libmsgxchng_${LIBMSGXCHNG_VERSION}_amd64.deb
...
libmsgxchng-18.04-docker: ubuntu-18.04
	docker build --build-arg LIBMSGXCHNG_VERSION=${LIBMSGXCHNG_VERSION} -t mubox/fpm-cookery:libmsgxchng-18.04 -f libmsgxchng/Dockerfile-18.04 libmsgxchng
...
pkg/deb/libmsgxchng_${LIBMSGXCHNG_VERSION}_18.04_amd64.deb: pkg/deb libmsgxchng-18.04-docker
	docker run --rm mubox/fpm-cookery:libmsgxchng-18.04 > pkg/deb/libmsgxchng_${LIBMSGXCHNG_VERSION}_18.04_amd64.deb
...
red-docker: ubuntu
	docker build --build-arg RED_VERSION=${RED_VERSION} --build-arg LIBBFRAME_VERSION=${LIBBFRAME_VERSION} --build-arg LIBMSGXCHNG_VERSION=${LIBMSGXCHNG_VERSION} -t mubox/fpm-cookery:red red
...
pkg/deb/red_${RED_VERSION}_amd64.deb: pkg/deb red-docker
	docker run --rm mubox/fpm-cookery:red > pkg/deb/red_${RED_VERSION}_amd64.deb
...
redd-docker: ubuntu
	docker build --build-arg REDD_VERSION=${REDD_VERSION} --build-arg LIBBFRAME_VERSION=${LIBBFRAME_VERSION} --build-arg LIBMSGXCHNG_VERSION=${LIBMSGXCHNG_VERSION} -t mubox/fpm-cookery:redd redd
...
pkg/deb/redd_${REDD_VERSION}_amd64.deb: pkg/deb redd-docker
	docker run --rm mubox/fpm-cookery:redd > pkg/deb/redd_${REDD_VERSION}_amd64.deb
